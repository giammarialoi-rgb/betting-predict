...
$status = Join-Path $RepoRoot "audit\external\task-039\collector-status.json"
if (Test-Path $lock) {
  try {
    $j = Get-Content $lock -Raw | ConvertFrom-Json
    if ($j.pid) {
      Stop-Process -Id $j.pid -Force -ErrorAction SilentlyContinue
      Write-Output "Stopped pid=$($j.pid)"
    }
  } catch {
    Write-Output "Could not parse lock: $_"
  }
...
Write-Output "Collector stop requested"
