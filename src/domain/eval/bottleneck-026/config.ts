...
import type { Exp026Config } from "@/domain/eval/bottleneck-026/types";

export const TASK026_CACHE_DIR = join(process.cwd(), "audit", "external", "task-026");

export function loadExp026Config(): Exp026Config {
  const raw = JSON.parse(
    readFileSync(join(process.cwd(), "experiments", "exp_026_break_bottleneck_v1.json"), "utf8"),
  ) as Exp026Config;
  if (raw.experiment_id !== "exp_026_break_bottleneck_v1") {
  ...
  if (
    ...
    throw new Error("ExperimentIntegrityError: frozen TASK 026 flags violated");
  }
...
export function assertHoldoutUntouched026(input: {
